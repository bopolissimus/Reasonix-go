...
var reTransientUserBlock = regexp.MustCompile(`(?s)^\s*<(?:reasoning-language|memory-update|background-jobs|nonce-context)>.*?</(?:reasoning-language|memory-update|background-jobs|nonce-context)>\s*\n?`)
